...
.board_list th {
	padding: 14px 0;
	background: url('<c:url value="/css/images/ico_board_th_bar.gif"/>') no-repeat 0 center
		#f5f5f5;
	font-size: 14px;
	color: #666;
	font-weight: 400;
...
<body>
	...
	<div style="width:100%; position:relative; left:calc(50% - 600px); height:600px; display:inline">
		...
		<div style="float:left; width:500px;">
			<table border="0">
				<tr>
					<td><img src="${pageContext.request.contextPath}/file/${list.GOODS_THUMBNAIL}" width="500"
						height="500" /></td>
				</tr>
			</table>
		</div>
...
<script type="text/javascript">
// context path for dynamic resources
var contextPath = '${pageContext.request.contextPath}';
// safe login flag for JS
var isLogin = '${SESSION_NO ne null ? "true" : "false"}';
// data values from JSP EL
var GOODS_NO = '${list.GOODS_NO}';
var GOODS_SELL_PRICE = ${list.GOODS_SELL_PRICE};
var COLOR_SIZE_COUNT = ${ColorSize};
var SIZE_COUNT = ${Sizecnt};
...
$(document).ready(function() {
    ...
    // Fix broken images in rich content: prefix context path for root-absolute src
    $('#home img').each(function(){
        var s = $(this).attr('src');
        if(!s) return;
        // If src starts with '/' and not already prefixed by contextPath, prefix it
        if(s.indexOf('http://') !== 0 && s.indexOf('https://') !== 0){
            if(s.charAt(0) === '/' && s.indexOf(contextPath + '/') !== 0){
                $(this).attr('src', contextPath + s);
            }
        }
    });
...
function fn_Qna(){ // 상품문의 작성
	...
	if(isLogin === 'true'){
		...
		comSubmit.setUrl(contextPath + "/shop/openQnaForm.do");
		comSubmit.addParam("IDX", GOODS_NO);
		comSubmit.submit();
	}else {
		alert("로그인 후 이용해주세요.");
...
function fn_Review(){
	...
	if(isLogin === 'true'){
		var comSubmit = new ComSubmit();
		comSubmit.setUrl(contextPath + "/shop/openReviewForm.do");
		comSubmit.addParam("IDX", GOODS_NO);
		comSubmit.submit();
	}else {
		alert("로그인 후 이용해주세요.");
...
function fn_update() {
	var comSubmit = new ComSubmit();
	comSubmit.setUrl(contextPath + "/shop/goodsModifyForm.do");
	comSubmit.addParam("IDX", GOODS_NO);
	comSubmit.submit();
}
...
function fn_InsertLike() { // 좋아요
	...
	if(isLogin === 'true'){
		var comSubmit = new ComSubmit();
		comSubmit.setUrl(contextPath + "/shop/goodsLike.do");
...
function fn_GoodsOrder() { // 구매하기
	...
	if(isLogin === 'true'){
		...
		if(len==0){
			...
			comSubmit.setUrl(contextPath + "/shop/goodsOrder.do");
			comSubmit.submit();
		}
	}else {
		alert("로그인 후 이용해주세요.");
...
function fn_InsertBasket() { // 장바구니
	...
	if(${SESSION_NO ne null}){
		...
		if(len==0){
			...
			var url = contextPath + "/shop/basketPopUp.do";
			var name = "popup";
			var option = "width=382, height=227, top=500, left=800, location=no";
			
		    var comSubmit = new ComSubmit("frm");
			comSubmit.setUrl(contextPath + "/shop/insertBasket.do");
...
$('#SizeList').change(function() {
	...
	return false;
		...
		var totalCount = COLOR_SIZE_COUNT * SIZE_COUNT;
		...
		html = " <table class='total_cartAdd' style='border:1px solid #bdbebd; width:600px; height:100px; margin-top:0px;' class='fieldx"+cnt+"'> "
		     ...
		     + " </div> "
		     + " <div class='product-line-price' style='float:left; margin-left:10px;' id='price_sell' price_sell='"+ GOODS_SELL_PRICE +"' >"+numberWithCommas(GOODS_SELL_PRICE)+"</div> "
		     + " <div class='product-removal"+cnt+"'> "
			 ...
			 + " <input type='hidden' name='IDX' id='IDX' value='"+GOODS_NO+"'> "
			 + " </table> ";
...
function fn_selectGoodsListCallback(data) {
	...
	if (total == 0) {
		...
		$.each(data.list, function(key, value) {
							    ...
							    var img = "<img src='"+ contextPath +"/img/ico_encryption.png'>"; 
    var Q= "<img src='"+ contextPath +"/img/ico_qna_q.png'>";
    var A= "<img src='"+ contextPath +"/img/ico_qna_a.png'>";
								if(Level == 1){//답변
									if(sc == 0){//공개
							str += "  "
								+  " <tr id='show"+cnt1+"'> "
